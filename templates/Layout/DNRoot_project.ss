...
<div class="content">
					...
					<% if $DNEnvironmentList %>
						<% loop $DNEnvironmentList %>
							<tr>
								...
								<td><% if $URL %>
									<a href="$URL">$BareURL</a><% else %>&nbsp;<% end_if %>
								</td>
								<td>
									<% if $CurrentBuild %>
	...
	<% if $CurrentProject.canViewCreateEnvironmentLogs %>
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						<% loop $CreateEnvironmentList %>
							<tr>
								<td><span class="tooltip-hint"
									data-toggle="tooltip"
									data-original-title="$LastEdited.Nice ($LastEdited.Ago)">$LastEdited.Nice</span>
								</td>
								<td>$Name</td>
								...
								<td class="text-center"><% if $Link %><a
									class="no-wrap"
									href="$Link">Details
									<i class="fa fa-angle-right"></i></a><% end_if %>
								</td>
							</tr>
						<% end_loop %>
					</tbody>
